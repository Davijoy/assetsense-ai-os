...
function leadContactLinks(l: { name: string; project: string; budget: string }) {
  const email = `${l.name.toLowerCase().replace(/\s+/g, ".")}@gmail.com`;
  const firstName = l.name.split(" ")[0];
  const phone = `+919${Array.from(l.name).reduce((acc, ch) => acc + ch.charCodeAt(0), 0).toString().slice(0, 9)}`;
  const digits = phone.replace(/\D/g, "");
  return {
    email,
    phone,
    tel: `tel:${phone}`,
    whatsapp: `https://wa.me/${digits}?text=${encodeURIComponent(`Hi ${firstName}, following up on ${l.project}.`)}`,
    mailto: `mailto:${email}?subject=${encodeURIComponent(`${l.project} — next steps`)}&body=${encodeURIComponent(`Hi ${firstName},\n\nFollowing up on your interest in ${l.project} (${l.budget}).`)}`,
  };
}
